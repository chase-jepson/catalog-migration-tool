...
function makeMappings(overrides: Record<string, string>): FieldMapping[] {
  const allFields = [
    'productIdentifier', 'productName', 'brand', 'productCategory',
    'productSubCategory', 'externalCategory', 'status', 'strain',
    'classification', 'weight', 'basePrice', 'description', 'menuTitle',
    'availableOnline', 'imageFilename', 'thc', 'cbd', 'tags', 'effects',
    'flavor', 'ingredients', 'variantIdentifier', 'priceTier', 'priceType',
  ];
...
describe('applyFixes', () => {
  ...
  it('changes category and cascades to subCategory, uom, merchSize', () => {
    ...
    const fixes: RowFix[] = [
      { rowIndex: 0, field: 'category', newValue: 'Merch' },
    ];
  ...
  it('applies classification fix', () => {
    const derived = getDerivedRows();
    const fixes: RowFix[] = [
      { rowIndex: 1, field: 'classification', newValue: 'Indica' },
    ];
  ...
  it('applies subCategory fix without cascading', () => {
    const derived = getDerivedRows();
    const fixes: RowFix[] = [
      { rowIndex: 0, field: 'subCategory', newValue: 'Shake' },
    ];
