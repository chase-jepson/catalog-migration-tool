...
describe("buildOutputCSVs", () => {
  ...
  it("brands CSV has correct headers and deduplicates brands", () => {
    ...
    for (let i = 0; i < 4; i++)
      rows.push(makeRow({ productId: `P - A${i}`, skuBarcode: `A${i}`, brand: "Alpha" }));
    for (let i = 0; i < 3; i++)
      rows.push(makeRow({ productId: `P - B${i}`, skuBarcode: `B${i}`, brand: "Beta" }));
    for (let i = 0; i < 3; i++)
      rows.push(makeRow({ productId: `P - G${i}`, skuBarcode: `G${i}`, brand: "Gamma" }));
