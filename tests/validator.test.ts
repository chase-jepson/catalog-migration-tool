...
describe('groupErrors', () => {
  it('groups errors by field + message with row indices', () => {
    const errors: RowValidationError[] = [
      { rowIndex: 0, field: 'category', currentValue: 'Bad', message: 'Invalid category: "Bad"', fixType: 'dropdown', dropdownOptions: [...PRODUCT_CATEGORIES], severity: 'error' },
      { rowIndex: 5, field: 'category', currentValue: 'Bad', message: 'Invalid category: "Bad"', fixType: 'dropdown', dropdownOptions: [...PRODUCT_CATEGORIES], severity: 'error' },
      { rowIndex: 1, field: 'productName', currentValue: '', message: 'Product name is empty', fixType: 'text', severity: 'error' },
    ];
  ...
  it('sorts groups by affected row count descending', () => {
    ...
    for (let i = 0; i < 47; i++) {
      errors.push({ rowIndex: i, field: 'category', currentValue: 'X', message: 'Invalid category: "X"', fixType: 'dropdown', severity: 'error' });
    }
    // 3 rows with bad name
    for (let i = 0; i < 3; i++) {
      errors.push({ rowIndex: i, field: 'productName', currentValue: '', message: 'Product name is empty', fixType: 'text', severity: 'error' });
    }
    const groups = groupErrors(errors);
  ...
  it('preserves severity and fixType in groups', () => {
    const errors: RowValidationError[] = [
      { rowIndex: 0, field: 'description', currentValue: '', message: 'Description is empty', fixType: 'text', severity: 'warning' },
    ];
    const groups = groupErrors(errors);
  ...
  it('preserves dropdownOptions in groups', () => {
    const errors: RowValidationError[] = [
      { rowIndex: 0, field: 'category', currentValue: 'X', message: 'Invalid category', fixType: 'dropdown', dropdownOptions: [...PRODUCT_CATEGORIES], severity: 'error' },
    ];
    const groups = groupErrors(errors);
