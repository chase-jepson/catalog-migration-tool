...
describe('processReceipts', () => {
  it('sums receipts by group and calculates UnitCost', () => {
    const receiptRows = [
      { 'Product SKU': 'SKU-1', 'External Package ID': 'PKG-1', 'Receive Date': '02/11/2026', 'Quantity': '10', 'Total Cost': '100', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
      { 'Product SKU': 'SKU-1', 'External Package ID': 'PKG-1', 'Receive Date': '02/11/2026', 'Quantity': '5', 'Total Cost': '50', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
    ];
    const inventoryExtPkgIds = new Set(['PKG-1']);

    const result = processReceipts(
      receiptRows, [], inventoryExtPkgIds,
      makeReceiptMappings(), makeAdjustmentMappings(),
    );
  ...
  it('stacks adjustments with receipts and re-sums', () => {
    ...
    const adjustmentRows = [
      { 'SerialNumber': 'PKG-1', 'qty': '-2', 'Cost': '-20' },
    ];
    const inventoryExtPkgIds = new Set(['PKG-1']);

    const result = processReceipts(
      receiptRows, adjustmentRows, inventoryExtPkgIds,
      makeReceiptMappings(), makeAdjustmentMappings(),
    );
  ...
  it('extracts InvoiceId from Order Title', () => {
    const receiptRows = [
      { 'Product SKU': 'SKU-1', 'External Package ID': 'PKG-1', 'Receive Date': '02/11/2026', 'Quantity': '10', 'Total Cost': '100', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-123 - INV456', 'Unit Cost': '' },
    ];
    const inventoryExtPkgIds = new Set(['PKG-1']);
  ...
  it('filters to active invoices that overlap with inventory', () => {
    const receiptRows = [
      ...
      { 'Product SKU': 'SKU-1', 'External Package ID': 'PKG-1', 'Receive Date': '02/11/2026', 'Quantity': '10', 'Total Cost': '100', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
      { 'Product SKU': 'SKU-2', 'External Package ID': 'PKG-2', 'Receive Date': '02/11/2026', 'Quantity': '5', 'Total Cost': '50', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
      // PKG-3 is NOT in inventory, different invoice
      { 'Product SKU': 'SKU-3', 'External Package ID': 'PKG-3', 'Receive Date': '01/01/2025', 'Quantity': '3', 'Total Cost': '30', 'Vendor Name': 'Vendor B', 'Order Title': 'PO-2 - INV002', 'Unit Cost': '' },
    ];
  ...
  it('filters out rows where Quantity = 0', () => {
    const receiptRows = [
      { 'Product SKU': 'SKU-1', 'External Package ID': 'PKG-1', 'Receive Date': '02/11/2026', 'Quantity': '10', 'Total Cost': '100', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
      { 'Product SKU': 'SKU-2', 'External Package ID': 'PKG-Z', 'Receive Date': '02/11/2026', 'Quantity': '0', 'Total Cost': '0', 'Vendor Name': 'Vendor A', 'Order Title': 'PO-1 - INV001', 'Unit Cost': '' },
    ];
...
describe('processInventory', () => {
  it('cleans THC/CBD via splitPotency', () => {
    const rows = [
      { 'SKU': 'S1', 'Product': 'P1', 'External package ID': 'PKG-1', 'Room': 'Sales Floor', 'Available': '10', 'Quantity (including allocated)': '10', 'Cost': '5', 'THC': '23.5 %', 'CBD': '0.00 mg', 'Available for': '', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
    ];
  ...
  it('derives customerType from Available for', () => {
    const rows = [
      { 'SKU': 'S1', 'Product': 'P1', 'External package ID': 'PKG-1', 'Room': 'Sales Floor', 'Available': '10', 'Quantity (including allocated)': '10', 'Cost': '5', 'THC': '', 'CBD': '', 'Available for': 'All enabled customer types', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
      { 'SKU': 'S2', 'Product': 'P2', 'External package ID': 'PKG-2', 'Room': 'Back Stock', 'Available': '5', 'Quantity (including allocated)': '5', 'Cost': '10', 'THC': '', 'CBD': '', 'Available for': 'Medical', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
    ];
  ...
  it('derives location path and sub-fields', () => {
    const rows = [
      { 'SKU': 'S1', 'Product': 'P1', 'External package ID': 'PKG-1', 'Room': 'Sales Floor', 'Available': '10', 'Quantity (including allocated)': '10', 'Cost': '5', 'THC': '', 'CBD': '', 'Available for': '', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
      { 'SKU': 'S2', 'Product': 'P2', 'External package ID': 'PKG-2', 'Room': 'Back Stock', 'Available': '5', 'Quantity (including allocated)': '5', 'Cost': '10', 'THC': '', 'CBD': '', 'Available for': '', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
    ];
...
describe('joinChain', () => {
  it('full joins inventory + invoice on ExternalPackageId', () => {
    const inventoryData = [
      { ExternalPackageId: 'PKG-1', Units: '10', ProductSKU: 'SKU-1' },
    ];
    const invoiceData = [
      { ExternalPackageId: 'PKG-1', Quantity: '15', InvoiceId: 'INV-1' },
    ];
  ...
  it('left joins with catalog on ProductSKU = ProductKey', () => {
    ...
    const catalogData = [
      { ProductKey: 'SKU-1', ProductCategory: 'Merch' },
    ];
...
describe('finalEnrichment', () => {
  ...
  it('assigns row numbers partitioned by ExternalPackageId', () => {
    const rows = [
      { ExternalPackageId: 'PKG-1', Units: '5', ProductSKU: 'SKU-1', ProductCategory: 'Flower', VendorName: '' },
      { ExternalPackageId: 'PKG-1', Units: '3', ProductSKU: 'SKU-2', ProductCategory: 'Flower', VendorName: '' },
      { ExternalPackageId: 'PKG-2', Units: '10', ProductSKU: 'SKU-3', ProductCategory: 'Flower', VendorName: '' },
    ];
  ...
  it('Merch TraceTreezId uses ExternalPackageId-RowNumber', () => {
    const rows = [
      { ExternalPackageId: 'PKG-1', Units: '5', ProductSKU: 'SKU-1', ProductCategory: 'Merch', VendorName: '' },
      { ExternalPackageId: 'PKG-1', Units: '3', ProductSKU: 'SKU-2', ProductCategory: 'Merch', VendorName: '' },
    ];
  ...
  it('Merch InventoryBarcodes is empty; non-Merch is ExternalPackageId', () => {
    const rows = [
      { ExternalPackageId: 'PKG-1', Units: '5', ProductSKU: 'SKU-1', ProductCategory: 'Merch', VendorName: '' },
      { ExternalPackageId: 'PKG-2', Units: '5', ProductSKU: 'SKU-2', ProductCategory: 'Flower', VendorName: '' },
    ];
  ...
  it('formats dates to yyyy-MM-dd', () => {
    const rows = [
      {
        ExternalPackageId: 'PKG-1', Units: '5', ProductSKU: 'SKU-1', ProductCategory: 'Flower', VendorName: '',
        ReceiveDate: '02/11/2026', HarvestDate: '1/5/2025', ExpirationDate: '12/31/2026', PackagedDate: '2/1/2025',
      },
    ];
...
describe('runInventoryETL', () => {
  it('produces InventoryDerivedRow[] from full fixture data', () => {
    const inventoryFile = makeParsedFile([
      { 'SKU': 'SKU-1', 'Product': 'Blue Dream', 'External package ID': 'PKG-1', 'Room': 'Sales Floor', 'Available': '10', 'Quantity (including allocated)': '10', 'Cost': '5', 'THC': '23.5 %', 'CBD': '1.2 mg', 'Available for': 'All enabled customer types', 'Expiration date': '12/31/2026', 'Packaging date': '1/15/2025', 'Harvest date': '1/1/2025', 'Category': 'Flower' },
    ]);
  ...
  it('gracefully degrades when only inventory file provided', () => {
    const inventoryFile = makeParsedFile([
      { 'SKU': 'SKU-1', 'Product': 'Blue Dream', 'External package ID': 'PKG-1', 'Room': 'Sales Floor', 'Available': '10', 'Quantity (including allocated)': '10', 'Cost': '5', 'THC': '23.5 %', 'CBD': '', 'Available for': '', 'Expiration date': '', 'Packaging date': '', 'Harvest date': '', 'Category': 'Flower' },
    ]);
