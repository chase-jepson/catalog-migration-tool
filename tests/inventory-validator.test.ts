...
describe("validateInventoryRows", () => {
  ...
  it("errors when both treezVariantId and variantReferenceId are empty", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "variantReferenceId" && e.severity === "error"),
    ).toBe(true);
  });

  it("passes when treezVariantId is set but variantReferenceId is empty", () => {
    const result = validateInventoryRows([
      makeRow({ treezVariantId: "TV-123", variantReferenceId: "" }),
    ]);
    const prodErrors = result.errors.filter(
      (e) => e.field === "variantReferenceId" && e.severity === "error",
    );
  ...
  it("errors on TraceTreezId that is not 24 chars or empty", () => {
    ...
    expect(
      result.errors.some(
        (e) => e.field === "traceTreezId" && e.message.includes("exactly 24 characters"),
      ),
    ).toBe(true);
  });
  ...
  it("errors on empty dispensaryLicense", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "dispensaryLicense" && e.severity === "error"),
    ).toBe(true);
  });
  ...
  it("errors when dispensaryLicense exceeds 45 chars", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "dispensaryLicense" && e.message.includes("45")),
    ).toBe(true);
  });
  ...
  it("errors on empty invoiceCreatedDate", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "invoiceCreatedDate" && e.severity === "error"),
    ).toBe(true);
  });
  ...
  it("errors on future invoiceCreatedDate", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "invoiceCreatedDate" && e.message.includes("future")),
    ).toBe(true);
  });
  ...
  it("warns when manifestNumber exceeds 45 chars", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "manifestNumber" && e.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("errors on empty originalUnitCount", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "originalUnitCount" && e.severity === "error"),
    ).toBe(true);
  });
  ...
  it("errors on non-whole originalUnitCount", () => {
    ...
    expect(
      result.errors.some(
        (e) => e.field === "originalUnitCount" && e.message.includes("whole number"),
      ),
    ).toBe(true);
  });
  ...
  it("errors on non-whole units", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "units" && e.message.includes("whole number")),
    ).toBe(true);
  });
  ...
  it("errors on negative unitCost", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "unitCost" && e.message.includes("negative")),
    ).toBe(true);
  });
  ...
  it("errors on invalid customerType", () => {
    ...
    expect(result.errors.some((e) => e.field === "customerType" && e.severity === "error")).toBe(
      true,
    );
  });
  ...
  it("errors when units > 0 but locationPath empty", () => {
    ...
    expect(result.errors.some((e) => e.field === "locationPath" && e.severity === "error")).toBe(
      true,
    );
  });
  ...
  it("errors on invalid locationInventoryType", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "locationInventoryType" && e.severity === "error"),
    ).toBe(true);
  });
  ...
  it("warns on invalid locationIsSellable", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "locationIsSellable" && e.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("errors on empty distributorName", () => {
    ...
    expect(result.errors.some((e) => e.field === "distributorName" && e.severity === "error")).toBe(
      true,
    );
  });
  ...
  it("errors when distributorName exceeds 255 chars", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorName" && e.message.includes("255")),
    ).toBe(true);
  });
  ...
  it("warns on invalid phone number", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorPhoneNumber" && e.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("warns on invalid email", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorEmail" && e.severity === "warning"),
    ).toBe(true);
  });

  it("errors on invalid distributorType", () => {
    const result = validateInventoryRows([makeRow({ distributorType: "Unknown" })]);
    expect(result.errors.some((e) => e.field === "distributorType" && e.severity === "error")).toBe(
      true,
    );
  });
  ...
  it("errors on invalid paymentTerm", () => {
    ...
    expect(
      result.errors.some(
        (e) => e.field === "distributorDefaultPaymentTerm" && e.severity === "error",
      ),
    ).toBe(true);
  });
  ...
  it("errors on lead time < 1", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorLeadTime" && e.severity === "error"),
    ).toBe(true);
  });

  it("errors on invalid paymentMethod", () => {
    const result = validateInventoryRows([
      makeRow({ distributorPreferredPaymentMethod: "BITCOIN" }),
    ]);
    expect(
      result.errors.some(
        (e) => e.field === "distributorPreferredPaymentMethod" && e.severity === "error",
      ),
    ).toBe(true);
  });
  ...
  it("errors on invalid license type", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorLicense1Type" && e.severity === "error"),
    ).toBe(true);
  });
  ...
  it("errors when license number present but type missing", () => {
    ...
    expect(
      result.errors.some(
        (e) =>
          e.field === "distributorLicense1Type" &&
          e.message.includes("required when License Number"),
      ),
    ).toBe(true);
  });
  ...
  it("errors when license number present but expiration missing", () => {
    ...
    expect(
      result.errors.some(
        (e) =>
          e.field === "distributorLicense1ExpirationDate" &&
          e.message.includes("missing Expiration Date"),
      ),
    ).toBe(true);
  });
  ...
  it("warns on invalid rep phone", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorRep1Phone" && e.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("warns on invalid rep email", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "distributorRep1Email" && e.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("warns on invalid harvestDate format", () => {
    ...
    expect(result.errors.some((e) => e.field === "harvestDate" && e.severity === "warning")).toBe(
      true,
    );
  });
  ...
  it("warns on empty externalPackageId", () => {
    ...
    expect(
      result.errors.some((e) => e.field === "externalPackageId" && e.severity === "warning"),
    ).toBe(true);
  });
...
describe("validateCrossRow", () => {
  ...
  it("flags multiple distributors within same invoice", () => {
    ...
    expect(
      errors.some(
        (e) => e.field === "distributorName" && e.message.includes("Multiple distributors"),
      ),
    ).toBe(true);
  });
  ...
  it("flags dispensaryLicense variation within invoice", () => {
    ...
    const errors = validateCrossRow(rows);
    expect(
      errors.some((e) => e.field === "dispensaryLicense" && e.message.includes("varies")),
    ).toBe(true);
  });
  ...
  it("flags invoiceCreatedDate variation within invoice", () => {
    ...
    const errors = validateCrossRow(rows);
    expect(
      errors.some((e) => e.field === "invoiceCreatedDate" && e.message.includes("varies")),
    ).toBe(true);
  });
  ...
  it("flags location field variation (case-insensitive path match)", () => {
    const rows = [
      ...
      makeRow({
        locationPath: "front of house",
        locationInventoryType: "Adult",
        invoiceId: "INV-2",
      }),
    ];
    const errors = validateCrossRow(rows);
  ...
  it("flags multiple dispensary licenses globally", () => {
    ...
    expect(
      errors.some(
        (e) => e.field === "dispensaryLicense" && e.message.includes("varies across CSV"),
      ),
    ).toBe(true);
  });
  ...
  it("flags traceTreezId appearing in multiple invoices", () => {
    ...
    const errors = validateCrossRow(rows);
    expect(
      errors.some((e) => e.field === "traceTreezId" && e.message.includes("multiple invoices")),
    ).toBe(true);
  });
