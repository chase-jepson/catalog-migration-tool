...
export function validateInventoryRows(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!row.treezVariantId && !row.variantReferenceId) {
      errors.push(err(i, 'variantReferenceId', row.variantReferenceId,
    ...
    if (row.traceTreezId && row.traceTreezId.length !== 24) {
      errors.push(err(i, 'traceTreezId', row.traceTreezId,
    ...
    if (!row.dispensaryLicense) {
      errors.push(err(i, 'dispensaryLicense', row.dispensaryLicense,
        'Dispensary License is required'));
    ...
    // ── Invoice ID: required ──
    if (!row.invoiceId) {
      errors.push(err(i, 'invoiceId', row.invoiceId,
        'Invoice ID is required'));
    }

    // ── Invoice Created Date: required, YYYY-MM-DD, not in future ──
    if (!row.invoiceCreatedDate) {
      errors.push(err(i, 'invoiceCreatedDate', row.invoiceCreatedDate,
        'Invoice Created Date is required'));
    ...
    if (row.manifestNumber && row.manifestNumber.length > FIELD_LIMITS.manifestNumber) {
      errors.push(err(i, 'manifestNumber', row.manifestNumber,
        `Manifest Number exceeds ${FIELD_LIMITS.manifestNumber} characters`, 'warning'));
    }

    // ── Original Unit Count: required, whole number >= 0 ──
    if (!row.originalUnitCount && row.originalUnitCount !== '0') {
      errors.push(err(i, 'originalUnitCount', row.originalUnitCount,
        'Original Unit Count is required'));
    ...
    if (row.units !== '') {
      if (!isWholeNumber(row.units)) {
        errors.push(err(i, 'units', row.units,
          `Units '${row.units}' must be a whole number`));
    ...
    if (!row.unitCost && row.unitCost !== '0') {
      errors.push(err(i, 'unitCost', row.unitCost,
        'Unit Cost is required'));
    } else if (!isNonNegativeDecimal(row.unitCost)) {
      if (!isValidDecimal(row.unitCost)) {
        errors.push(err(i, 'unitCost', row.unitCost,
          `Unit Cost '${row.unitCost}' is not a valid number`));
      } else {
        errors.push(err(i, 'unitCost', row.unitCost,
    ...
    if (row.cbdAmount) {
      if (!isNonNegativeDecimal(row.cbdAmount)) {
        errors.push(err(i, 'cbdAmount', row.cbdAmount,
          `CBD Amount '${row.cbdAmount}' must be a non-negative number`));
      } else if (!row.cbdUom) {
        errors.push(err(i, 'cbdUom', row.cbdUom,
    ...
    if (!Number.isNaN(unitsNum) && unitsNum > 0 && !row.locationPath) {
      errors.push(err(i, 'locationPath', row.locationPath,
        'Location Path is required when Units > 0'));
    }

    // ── Location Inventory Type: enum ──
    if (row.locationInventoryType && !matchesEnum(row.locationInventoryType, LOCATION_INVENTORY_TYPE_VALUES)) {
      errors.push(err(i, 'locationInventoryType', row.locationInventoryType,
        `Location Inventory Type '${row.locationInventoryType}' not valid. Expected: ${LOCATION_INVENTORY_TYPE_VALUES.join(', ')}`,
        'error', 'dropdown', [...LOCATION_INVENTORY_TYPE_VALUES]));
    }

    // ── Location Is Sellable: TRUE/FALSE (warning) ──
    if (row.locationIsSellable && !matchesEnum(row.locationIsSellable, BOOLEAN_VALUES)) {
      errors.push(err(i, 'locationIsSellable', row.locationIsSellable,
        `Location Is Sellable must be TRUE or FALSE`, 'warning', 'dropdown', [...BOOLEAN_VALUES]));
    }

    // ── Location Default Receiving: TRUE/FALSE (warning) ──
    if (row.locationDefaultReceivingLocation && !matchesEnum(row.locationDefaultReceivingLocation, BOOLEAN_VALUES)) {
      errors.push(err(i, 'locationDefaultReceivingLocation', row.locationDefaultReceivingLocation,
        `Location Default Receiving Location must be TRUE or FALSE`, 'warning', 'dropdown', [...BOOLEAN_VALUES]));
    }

    // ── Distributor Name: required, max 255 chars ──
    if (!row.distributorName) {
      errors.push(err(i, 'distributorName', row.distributorName,
    ...
    if (row.distributorDBA && row.distributorDBA.length > FIELD_LIMITS.distributorDBA) {
      errors.push(err(i, 'distributorDBA', row.distributorDBA,
        `Distributor DBA exceeds ${FIELD_LIMITS.distributorDBA} characters`, 'warning'));
    }

    // ── Distributor Address: max 255 ──
    if (row.distributorAddress && row.distributorAddress.length > FIELD_LIMITS.distributorAddress) {
      errors.push(err(i, 'distributorAddress', row.distributorAddress,
        `Distributor Address exceeds ${FIELD_LIMITS.distributorAddress} characters`, 'warning'));
    }
    ...
    if (row.distributorPhoneNumber) {
      ...
      if (!phone.valid) {
        errors.push(err(i, 'distributorPhoneNumber', row.distributorPhoneNumber,
          `Phone '${row.distributorPhoneNumber}' must be 10 digits`, 'warning'));
      }
    }
    ...
    if (row.distributorEmail) {
      if (row.distributorEmail.length > FIELD_LIMITS.distributorEmail) {
        errors.push(err(i, 'distributorEmail', row.distributorEmail,
          'Distributor Email exceeds 100 characters', 'warning'));
      } else if (!EMAIL_REGEX.test(row.distributorEmail)) {
        errors.push(err(i, 'distributorEmail', row.distributorEmail,
          `Email '${row.distributorEmail}' is not valid`, 'warning'));
      }
    }

    // ── Distributor Type: enum ──
    if (row.distributorType && !matchesEnum(row.distributorType, DISTRIBUTOR_TYPE_VALUES)) {
      errors.push(err(i, 'distributorType', row.distributorType,
        `Distributor Type '${row.distributorType}' not valid. Expected: ${DISTRIBUTOR_TYPE_VALUES.join(', ')}`,
        'error', 'dropdown', [...DISTRIBUTOR_TYPE_VALUES]));
    }

    // ── Distributor Default Payment Term: enum ──
    if (row.distributorDefaultPaymentTerm && !matchesEnum(row.distributorDefaultPaymentTerm, PAYMENT_TERM_VALUES)) {
      errors.push(err(i, 'distributorDefaultPaymentTerm', row.distributorDefaultPaymentTerm,
        `Payment Term '${row.distributorDefaultPaymentTerm}' not valid. Expected: ${PAYMENT_TERM_VALUES.join(', ')}`,
        'error', 'dropdown', [...PAYMENT_TERM_VALUES]));
    }
    ...
    if (row.distributorLeadTime) {
      ...
      if (!Number.isInteger(lt) || lt < 1) {
        errors.push(err(i, 'distributorLeadTime', row.distributorLeadTime,
          `Lead Time '${row.distributorLeadTime}' must be an integer >= 1`));
      }
    }

    // ── Distributor Preferred Payment Method: enum ──
    if (row.distributorPreferredPaymentMethod && !matchesEnum(row.distributorPreferredPaymentMethod, PAYMENT_METHOD_VALUES)) {
      errors.push(err(i, 'distributorPreferredPaymentMethod', row.distributorPreferredPaymentMethod,
        `Payment Method '${row.distributorPreferredPaymentMethod}' not valid. Expected: ${PAYMENT_METHOD_VALUES.join(', ')}`,
        'error', 'dropdown', [...PAYMENT_METHOD_VALUES]));
    }
    ...
    for (const n of [1, 2, 3] as const) {
      ...
      if (typeVal && !matchesEnum(typeVal, LICENSE_TYPE_VALUES)) {
        errors.push(err(i, typeField, typeVal,
          `License ${n} Type '${typeVal}' not valid. Expected: ${LICENSE_TYPE_VALUES.join(', ')}`,
          'error', 'dropdown', [...LICENSE_TYPE_VALUES]));
      }
      ...
      if (numberVal && !expVal) {
        errors.push(err(i, expField, expVal,
    ...
    for (const n of [1, 2, 3] as const) {
      ...
      if (phoneVal) {
        ...
        if (!phone.valid) {
          errors.push(err(i, phoneField, phoneVal,
            `Rep ${n} Phone '${phoneVal}' must be 10 digits`, 'warning'));
        }
      }
      if (emailVal) {
        const limit = FIELD_LIMITS[`distributorRep${n}Email` as keyof typeof FIELD_LIMITS] ?? 100;
        if (emailVal.length > limit) {
          errors.push(err(i, emailField, emailVal,
            `Rep ${n} Email exceeds 100 characters`, 'warning'));
        } else if (!EMAIL_REGEX.test(emailVal)) {
          errors.push(err(i, emailField, emailVal,
            `Rep ${n} Email '${emailVal}' is not valid`, 'warning'));
        }
      }
    }
    ...
    for (const dateField of ['harvestDate', 'expirationDate', 'packagedDate'] as const) {
      ...
      if (val && !DATE_REGEX.test(val)) {
        errors.push(err(i, dateField, val,
          `Invalid date format (expected YYYY-MM-DD): ${val}`, 'warning'));
      }
    }

    // ── ExternalPackageId: warn if empty ──
    if (!row.externalPackageId) {
      errors.push(err(i, 'externalPackageId', row.externalPackageId,
        'External Package ID is empty', 'warning'));
    }
  }
...
function fmtRows(indices: number[], maxShow = 5): string {
  ...
  return sorted.slice(0, maxShow).map((r) => String(r + 1)).join(', ') + ` ... (${sorted.length} total)`;
}
  ...
  for (const [, pkgRows] of groups) {
    ...
    for (const row of pkgRows.slice(1)) {
      ...
      if (row.originalUnitCount !== first.originalUnitCount) {
        errors.push(err(rowIdx, 'originalUnitCount', row.originalUnitCount,
      ...
      if (row.unitCost !== first.unitCost) {
        errors.push(err(rowIdx, 'unitCost', row.unitCost,
          `Invoice '${first.invoiceId}', TraceTreezId '${first.traceTreezId}': ` +
      ...
      if (row.customerType.toUpperCase() !== first.customerType.toUpperCase()) {
        errors.push(err(rowIdx, 'customerType', row.customerType,
          `Invoice '${first.invoiceId}', TraceTreezId '${first.traceTreezId}': ` +
    ...
    if (totalUnits > origCount) {
      const detail = pkgRows.map((r) =>
        `row ${originalIndex(allRows, r) + 1}: ${r.units || 0} units`,
      ).join(', ');
      errors.push(err(firstIdx, 'units', String(totalUnits),
        `Invoice '${first.invoiceId}', TraceTreezId '${first.traceTreezId}': ` +
  ...
  for (const [invoiceId, invRows] of groups) {
    ...
    if (distNames.size > 1) {
      const detail = Array.from(distNames.entries())
        ...
        .join('; ');
      errors.push(err(originalIndex(allRows, invRows[0]), 'distributorName', '',
    ...
    if (licenses.size > 1) {
      const detail = Array.from(licenses.entries())
        ...
        .join('; ');
      errors.push(err(originalIndex(allRows, invRows[0]), 'dispensaryLicense', '',
    ...
    if (dates.size > 1) {
      const detail = Array.from(dates.entries())
        ...
        .join('; ');
      errors.push(err(originalIndex(allRows, invRows[0]), 'invoiceCreatedDate', '',
...
const DISTRIBUTOR_LICENSE_FIELDS: (keyof InventoryDerivedRow)[] = [
  'distributorLicense1Type', 'distributorLicense1Number', 'distributorLicense1ExpirationDate',
  'distributorLicense2Type', 'distributorLicense2Number', 'distributorLicense2ExpirationDate',
  'distributorLicense3Type', 'distributorLicense3Number', 'distributorLicense3ExpirationDate',
];

const DISTRIBUTOR_REP_FIELDS: (keyof InventoryDerivedRow)[] = [
  'distributorRep1Name', 'distributorRep1Phone', 'distributorRep1Email', 'distributorRep1Role', 'distributorRep1Notes',
  'distributorRep2Name', 'distributorRep2Phone', 'distributorRep2Email', 'distributorRep2Role', 'distributorRep2Notes',
  'distributorRep3Name', 'distributorRep3Phone', 'distributorRep3Email', 'distributorRep3Role', 'distributorRep3Notes',
];
...
function _validateDistributorConsistency(
  ...
  for (const [, distRows] of distMap) {
    ...
    for (const { field, display } of DISTRIBUTOR_CONSISTENCY_FIELDS) {
      ...
      for (const row of distRows.slice(1)) {
        if (row[field] !== firstVal) {
          const rowIdx = originalIndex(allRows, row);
          errors.push(err(rowIdx, field, String(row[field]),
    ...
    for (const row of distRows.slice(1)) {
      ...
      if (differs) {
        const rowIdx = originalIndex(allRows, row);
        errors.push(err(rowIdx, 'distributorLicense1Number', '',
    ...
    for (const row of distRows.slice(1)) {
      ...
      if (differs) {
        const rowIdx = originalIndex(allRows, row);
        errors.push(err(rowIdx, 'distributorRep1Name', '',
...
function _validateGlobalDispensaryLicense(
  ...
  if (licenses.size > 1) {
    errors.push(err(-1, 'dispensaryLicense', '',
...
function _validateTraceTreezIdCrossInvoice(
  ...
  for (const [ttid, invIds] of ttidInvoices) {
    if (invIds.size > 1) {
      ...
      errors.push(err(originalIndex(allRows, firstRow), 'traceTreezId', ttid,
...
export function mapPortalIssuesToErrors(issues: PortalIssue[]): RowValidationError[] {
  return issues.map((issue) => {
    ...
    const field = issue.field_name ? (COLUMN_TO_FIELD[issue.field_name] ?? issue.field_name) : 'unknown';
